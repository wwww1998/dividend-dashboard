# -*- coding: utf-8 -*-
"""读 result_3y.json, 生成 dividend_dashboard_3y.html 网页看板(近3年)"""
import json, datetime

r = json.load(open("result_3y.json", encoding="utf-8"))
idx = r["indices"]
...
<title>四大红利全收益指数 · 3年定投数据回测</title>
...
<!-- HERO -->
<div class="hero">
  <h1>每月定投 %%AMOUNT%% 元红利指数，3 年后能赚多少？<br><em>四大红利全收益指数 ·数据回测</em></h1>
  <div class="sub">基于中证指数官网全收益指数官方日线（csindex.com.cn），逐日复算 2023-08 至 2026-07 三年定投：每月首个交易日买入 %%AMOUNT%% 元，共 %%PERIODS%% 期、累计投入 ¥%%TOTAL%%。重点还原每个指数的<b style="color:#ffb37a">真实回测</b>——包括定投组合的实际浮亏与恢复耗时。</div>
  <div class="rules">
    <div class="rule">周期 <b>2023.08 - 2026.07</b></div>
  ...
  <div class="know">
    ...
    <div class="kn">
      ...
      <ul>
        ...
        <li><b>复利滚雪球（分红再投资＋定投）</b>：分红再买份额、跌时定投买更多份额，份额越滚越多。回测里 3 年 %%INVEST_WAN%% 变 %%FINAL_BEST_WAN%%（红利低波），一半收益来自"低点坚持买入积累的份额"。</li>
        <li><b>防御性（低波动抗跌）</b>：成分股多为银行、煤炭、石化、交运龙头，盈利稳定。3 年定投最大回撤仅 -10.0%~-12.5%，比大盘温和。</li>
      </ul>
...
<div class="sec">
  ...
  <div class="alert">
    <div class="t">⚠ 持有体验提醒：2023 年以来最深的坑在哪</div>
    <div class="b">定投区间内最深的坑是 <b>2024 年 5-9 月回调</b>（指数口径最大回撤 %%MDD_IDX_RANGE%%）与 <b>2026 年 6 月回调</b>（组合口径浮亏 %%MDD_AMT_RANGE%% 元）。定投通过持续买入摊低成本，把这些坑填浅了一大半——但请记住：回撤是红利策略的固有属性，不是 bug。</div>
...
<div class="sec">
  ...
  <div class="desc">灰线为累计投入本金（阶梯至 %%TOTAL%%）。曲线在大部分年份都压在投入线上方，但 2024 年 5-9 月与 2026 年 6 月回调时会短暂跌破——"亏钱时咬牙继续投"正是收益的主要来源。</div>
  <div class="chart">
...
<div class="sec">
  ...
  <div class="desc">下拉切换指数。红色柱＝年末账户浮盈，绿色柱＝年末浮亏（2023 年定投初期四指数曾小幅浮亏、2024 年 5-9 月回调亦是摊低成本的黄金期）。<b>「指数年度涨跌幅」＝该指数全收益口径自然年涨幅</b>，用来对照"账户当年盈亏有多少来自指数本身、多少来自定投摊低成本"。</div>
  <div class="chart">
    <div class="head">
...
<div class="sec">
  ...
  <div class="desc">各指数年度涨跌幅（<b>全收益口径</b>，含分红再投）。2023 年为定投起点（2023-08-01）至年末，其余年份为自然年（上年末→本年末）；2026 行为截至 2026-07-31（非完整年度）。红涨绿跌（A 股习惯）。</div>
...
<!-- 6 那到底选哪个 -->
  ...
  <div class="quote">
    <p>「3 年定投的最大考验从来不是选哪个指数，而是在账面浮亏的那几个月里，你能不能继续投下去。定投最大的敌人不是市场，是你自己。」</p>
...
<div class="foot">
  ...
  <p>2. <b>定投规则</b>：每月首个交易日按当日收盘价买入 %%AMOUNT%% 元；2023-08 首期至 2026-07 末期为 %%PERIODS%% 期、总投入 ¥%%TOTAL%%；未计交易费用与税费。</p>
  ...
  CARDS.forEach((c,i)=>{
    ...
    el.innerHTML = `
      ...
      <div class="kv">
        ...
        <div><div class="k">指数3年最大回撤</div><div class="v neg">${c.mdd_i.toFixed(2)}%</div></div>
      </div>
...
(function(){
  ...
  const items = [
    {n:"指数口径最深回撤（2024 年 5-9 月回调）", v:`${worst.pct.toFixed(2)}%`, s:`${worst.short} · ${worst.peak} → ${worst.trough}，约 ${worst.days/30.44|0} 个月收复`},
    {n:"组合口径最大浮亏", v:`-¥${worstPort.mdd_p_amt.toLocaleString()}`, s:`${worstPort.short} · 峰值 ${worstPort.mdd_p_peak} → 谷底 ${worstPort.mdd_p_trough}`},
    {n:"四个指数平均组合回撤", v:`${(CARDS.reduce((s,c)=>s+c.mdd_p,0)/4).toFixed(2)}%`, s:`2023.08 - 2026.07 定投区间内`},
    {n:"组合回撤最深的时点", v:`2026.06`, s:`2026 年 6 月回调，谷底浮亏 ${CARDS.filter(c=>c.mdd_p_trough.startsWith("2026-06")).map(c=>SHORT[c.code]+" -¥"+c.mdd_p_amt.toLocaleString()).join("、")}`},
  ];
  items.forEach(it=>{
    const el = document.createElement("div");
    el.className = "ag";
...
(function(){
  ...
  const html = PICK.map(code=>{
    ...
    const li = [
      ...
      ["3年终值", "¥"+c.final_value.toLocaleString()+"（+"+c.total_ret.toFixed(1)+"%）"],
...
open("dividend_dashboard_3y.html", "w", encoding="utf-8").write(html)
print("已生成 dividend_dashboard_3y.html,", len(html)//1024, "KB")

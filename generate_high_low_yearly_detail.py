# -*- coding: utf-8 -*-
"""
四大红利全收益指数 · 每年最高价 vs 最低价 曲线对比
加入每年首个交易日定投曲线作为中间参考
10年/5年/3年/1年 独立页面
"""
import json

YEARS = [(10, {"label": "10年", "start": "2017", "end": "2026.07", "periods": "10期", "invest": "¥100万", "result_fn": "result_yearly.json"}),
         (5,  {"label": "5年",  "start": "2022", "end": "2026.07", "periods": "5期",  "invest": "¥50万",  "result_fn": "result_yearly_5y.json"}),
         (3,  {"label": "3年",  "start": "2024", "end": "2026.07", "periods": "3期",  "invest": "¥30万",  "result_fn": "result_yearly_3y.json"}),
         (1,  {"label": "1年",  "start": "2025", "end": "2026.07", "periods": "1期",  "invest": "¥10万",  "result_fn": "result_yearly_1y.json"})]
...
for y, meta in YEARS:
    fn = f"high_low_yearly_{y}y.json"
    ...
    for c in CODES:
        ...
        # 缩放首日数据到与high_low数据同量级（投资金额不同）
        hl_total = idx["high"]["final_value"] + idx["low"]["final_value"]  # 近似
        fd_total = r_idx["total_invest"]
        hl_total_invest = data["meta"]["total_invest"]
        scale = hl_total_invest / fd_total if fd_total > 0 else 1
        fd_value_scaled = [v * scale for v in fd_plot["value"]]
        ...
        chart_data[c] = {
            ...
            "firstday_value": fd_value_scaled,
            ...
            "firstday_final": round(fd_final * scale),
...
<title>四大红利 · {meta['label']}每年最高价vs最低价定投</title>
...
.hero{{background:linear-gradient(135deg,#1a3a2a 0%,#1d2939 60%,#2b3a3a 100%);border-radius:18px;color:#fff;padding:38px 40px;position:relative;overflow:hidden}}
...
.hero h1 em{{font-style:normal;color:#7fdb9a}}
...
<div class="hero">
  <h1>四大红利·{meta['label']}每年最高价 vs 最低价<br><em>每年年初定投 · 含首日定投曲线 · 真实回撤</em></h1>
  <div class="sub">每年年初投入固定金额，分别在当年最高价买入、最低价买入、以及每年首个交易日买入。择时对一次性大额投入的影响远大于每月定投。</div>
  <div class="rules">
    <div class="rule">周期 <b>{meta['start']} - {meta['end']}</b></div>
    <div class="rule">{meta['periods']} · 每期 ¥100,000</div>
...
<div class="sec">
  <h2>总结：{meta['label']}每年定投，择时影响</h2>
  <div class="desc">{meta['label']}期间，三种策略的年化收益和终值差异。一次性大额投入的择时影响远大于每月定投。</div>
...
<div class="foot">
  ...
  <p>2. <b>定投规则</b>：每年年初投入100,000元，分别在当年最高价买入、当年最低价买入、每年首个交易日买入；未计交易费用与税费。</p>
...
(function(){{
  ...
  CODES.forEach(c => {{
    ...
    outname = f"dividend_high_low_yearly_detail_{y}y.html"

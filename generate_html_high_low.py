# -*- coding: utf-8 -*-
"""读 high_low_10y.json, 生成网页看板：每月最高价/最低价定投对比"""
import json, datetime

r = json.load(open("high_low_10y.json", encoding="utf-8"))
idx = r["indices"]
...
<title>四大红利全收益指数 · 每月最高/最低价定投对比</title>
...
<!-- HERO -->
<div class="hero">
  <h1>每月最高价 vs 最低价定投 · 四大红利指数<br><em>对比：10年回测 · 含分红再投 · 真实回撤计算</em></h1>
  <div class="sub">基于中证指数官网全收益指数官方日线（csindex.com.cn），逐日复算 2016-08 至 2026-07 十年定投：每月定投固定金额，分别在当月最高价买入 vs 当月最低价买入，对比终值和回撤差异。</div>
  <div class="rules">
    <div class="rule">周期 <b>2016.08 - 2026.07</b></div>
    <div class="rule">频率 <b>每月定投</b></div>
    <div class="rule">每期 <b>¥10,000</b></div>
    <div class="rule">总投入 <b>¥1,200,000</b>（120 期）</div>
...
<div class="sec">
  ...
  <div class="desc">择时定投"等跌了再买"，本质就是想尽量买在当月偏低位置。回测告诉你：十年累计下来影响有多大。</div>
  <div class="know">
    <div class="kn">
...
<!-- 1 总账对比 -->
<div class="sec">
  <h2><span class="no">1</span>总账：最高价 vs 最低价，十年后差多少</h2>
  <div class="desc">每个指数分别展示最高价买入和最低价买入的终值、收益、回撤对比。</div>
...
<div class="sec">
  ...
  <div class="desc">四个指数从定投起点 2016-08-01 归一化为 100，走势对比一目了然。</div>
  <div class="card">
    <div id="c_trend"></div>
...
<div class="sec">
  ...
  <div class="desc">十年回测告诉你，每月都能买到最低点这种完美择时，长期到底能多赚多少。</div>
  <div class="pick" id="pick"></div>
  <div class="quote">
    <p>「长期来看，就算你每次都能精准买在当月最低点，比"闭着眼睛买在第一个交易日"也只多赚不到 10%。定投最大的收益来自「坚持买入」，而非「买在最低点」。」</p>
    <div class="who">—— 基于中证指数官方全收益日线十年回测</div>
  </div>
</div>
...
<div class="foot">
  ...
  <p>2. <b>定投规则</b>：每月固定投入 10,000 元，分别在当月最高价买入 vs 当月最低价买入；2016-08 首期至 2026-07 末期为 120 期、总投入 ¥1,200,000；未计交易费用与税费。</p>
...
(function(){
  ...
  INDICES.forEach((it)=>{
    const h = it.high;
    const l = it.low;
    const diff = (l.final_value - h.final_value) / h.final_value * 100;
    ...
    el.innerHTML = `
      ...
      <div class="cards-2col" style="gap:10px">
        ...
        <div>
          ...
          <div class="kv">
            <div><div class="k">终值差</div><div class="v pos">+${diff.toFixed(1)}%</div></div>
...
const __chartTasks = [];
setTimeout(function(){
  function __run(i){ if(i < __chartTasks.length) { __chartTasks[i](); setTimeout(()=>__run(i+1), 50); } }
  __run(0);
}, 100);
...
(function(){
  ...
  INDICES.forEach(it=>{
    const h = it.high;
    const l = it.low;
    const diff = (l.total_ret - h.total_ret) * 100;
    const c = COLORS[it.code];
    const el = document.createElement("div");
    el.className = "pc";
    el.innerHTML = `
      ...
      <div class="pc-sub">十年总收益差异（最低价买入 vs 最高价买入）</div>
      <ul>
...
outname = "dividend_high_low_monthly_10y.html"

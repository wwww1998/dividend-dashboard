# -*- coding: utf-8 -*-
"""
四大红利全收益指数 5年定投回测 + 真实回撤计算
规则: 每月第一个交易日定投10000元, 2021-08 ~ 2026-07, 共60期, 总投入6万
输出: result_5y.json (供网页使用)
"""
...
AMOUNT = 10000.0
START, END = "2021-08", "2026-07"   # 定投区间(近5年)
...
for idx in INDICES:
    ...
    for p in port_series:
        ...
        years[y] = p
    # 指数年度涨跌幅(全收益, 用全史数据)
    # 口径: 2021年为定投起点(2021-08-01)至年末, 其余年份为自然年(上年末→本年末)
    ...
    idx_rets = {}
    if "2021" in by_yr:
        base_2021 = close_map.get("2021-08-01") or by_yr["2020"]
        idx_rets["2021"] = round(by_yr["2021"] / base_2021 - 1, 4)
...
json.dump(result, open("result_5y.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)

# 打印摘要
print(f"{'指数':<12}{'终值':>10}{'总收益':>10}{'年化':>8}{'组合MDD':>9}{'指数5yMDD':>11}{'指数全史MDD':>12}")
for idx in result["indices"]:
    md = idx["mdd_port"]["pct"]

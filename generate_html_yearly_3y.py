# -*- coding: utf-8 -*-
"""读 result_yearly_3y.json, 生成 dividend_dashboard_yearly_3y.html 网页看板(每年定投3年)"""
import json, datetime

r = json.load(open("result_yearly_3y.json", encoding="utf-8"))
idx = r["indices"]
...
<title>四大红利全收益指数 · 3年每年定投数据回测</title>
...
<!-- HERO -->
<div class="hero">
  <h1>每年年初定投 %%AMOUNT%% 元红利指数，3 年后能赚多少？<br><em>四大红利全收益指数 ·数据回测</em></h1>
  <div class="sub">基于中证指数官网全收益指数官方日线（csindex.com.cn），逐日复算 2024-01 至 2026-07 三年定投：每年年初（1 月首个交易日）买入 %%AMOUNT%% 元，共 %%PERIODS%% 期、累计投入 ¥%%TOTAL%%。重点还原每个指数的<b style="color:#ffb37a">真实回测</b>——包括定投组合的实际浮亏与恢复耗时。</div>
  <div class="rules">
    <div class="rule">周期 <b>2024.01 - 2026.07</b></div>
  ...
  <div class="know">
    ...
    <div class="kn">
      ...
      <ul>
        ...
        <li><b>防御性（低波动抗跌）</b>：成分股多为银行、煤炭、石化、交运龙头，盈利稳定。3 年定投最大回撤仅 -13.5%~-14.5%，比大盘温和。</li>
      </ul>
...
<div class="sec">
  ...
  <div class="alert">
    <div class="t">⚠ 持有体验提醒：2024 年以来最深的坑在哪</div>
...
<div class="sec">
  ...
  <div class="desc">下拉切换指数。红色柱＝年末账户浮盈，绿色柱＝年末浮亏（2024 年 5-9 月回调、2026 年 3-6 月回落亦是摊低成本的黄金期，但三年年末全部盈利）。<b>「指数年度涨跌幅」＝该指数全收益口径自然年涨幅</b>，用来对照"账户当年盈亏有多少来自指数本身、多少来自定投摊低成本"。</div>
  <div class="chart">
    <div class="head">
...
<div class="foot">
  ...
  <p>2. <b>定投规则</b>：每年年初（1 月首个交易日）按当日收盘价买入 %%AMOUNT%% 元；2024-01 首期至 2026-01 末期为 %%PERIODS%% 期、总投入 ¥%%TOTAL%%；未计交易费用与税费。</p>
...
(function(){
  ...
  const items = [
    ...
    {n:"四个指数平均组合回撤", v:`${(CARDS.reduce((s,c)=>s+c.mdd_p,0)/4).toFixed(2)}%`, s:`2024.01 - 2026.07 定投区间内`},
...
open("dividend_dashboard_yearly_3y.html", "w", encoding="utf-8").write(html)
print("已生成 dividend_dashboard_yearly_3y.html,", len(html)//1024, "KB")

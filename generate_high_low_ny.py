...
def get_start_end(years):
    ...
    start_m = 8
    if years == 10:
        start_y = 2016; start_m = 8
    elif years == 5:
        start_y = 2021; start_m = 8
    elif years == 3:
        start_y = 2023; start_m = 8
    elif years == 1:
        start_y = 2025; start_m = 8
...
def compute(seg, start, end, amount):
    ...
    for p in seg:
        d = p["date"]
        ym = d[:7]
        is_buy = d in [v[0] for v in first.values()]
...
# ========== 主计算 ==========
if len(sys.argv) != 2:
    print("Usage: python generate_high_low_ny.py N")
    print("N: 1, 3, 5, 10")
    sys.exit(1)

years = int(sys.argv[1])
start, end = get_start_end(years)
print(f"区间 {start} - {end}, 年限 {years} 年")
...
outfn = f"high_low_{years}y.json"

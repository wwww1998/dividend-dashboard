# -*- coding: utf-8 -*-
"""
四大红利全收益指数 3年定投回测 + 真实回撤计算
规则: 每月第一个交易日定投10000元, 2023-08 ~ 2026-07, 共36期, 总投入3.6万
输出: result_3y.json (供网页使用)
"""
...
AMOUNT = 10000.0
START, END = "2023-08", "2026-07"   # 定投区间(近3年)
...
for idx in INDICES:
    ...
    for p in port_series:
        ...
        years[y] = p
    # 指数年度涨跌幅(全收益, 用全史数据)
    # 口径: 2023年为定投起点(2023-08-01)至年末, 其余年份为自然年(上年末→本年末)
    ...
    idx_rets = {}
    if "2023" in by_yr:
        base_2023 = close_map.get("2023-08-01") or by_yr["2022"]
        idx_rets["2023"] = round(by_yr["2023"] / base_2023 - 1, 4)
...
json.dump(result, open("result_3y.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
